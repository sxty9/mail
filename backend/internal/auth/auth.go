...
// AppVerifier checks an app password for a user (implemented by internal/apppass). Kept as
// an interface so auth stays free of a storage dependency.
type AppVerifier interface {
	Verify(user, token string) bool
}

// AppUser authenticates a native client via HTTP Basic auth (username = Linux account,
// password = an app password). Groups + admin are resolved live from the OS exactly as for a
// session, so rights enforcement is identical. Returns ErrNoSession on any failure.
func (v *Verifier) AppUser(r *http.Request, av AppVerifier) (*User, error) {
	user, pass, ok := r.BasicAuth()
	if !ok || user == "" || pass == "" {
		return nil, ErrNoSession
	}
	if !av.Verify(user, pass) {
		return nil, ErrNoSession
	}
	groups, exists := resolveGroups(user)
	if !exists {
		return nil, ErrNoSession
	}
	return &User{Username: user, Groups: groups, IsAdmin: contains(groups, v.adminGroup)}, nil
}
